describe "Console scrolling", ->
  ...
  xit "should move lines towards top of 'screen' as lines are added at bottom", ->
    ...
    expect(@scroll.length).toEqual(23)
    expect(@scroll[22]).toEqual("230 X=130")

    @console.println("240 X=140")
    expect(@scroll.length).toEqual(23)
    expect(@scroll[22]).toEqual("240 X=140")
    expect(@scroll[0]).toEqual("20 Y=33")
    expect(@scroll[1]).toEqual("30 Z=888")

    @console.println("250 X=150")
    expect(@scroll.length).toEqual(23)
    expect(@scroll[22]).toEqual("250 X=150")
    expect(@scroll[0]).toEqual("30 Z=888")
    expect(@scroll[1]).toEqual("40 M=X+Y")

    @console.println("260 X=160")
    expect(@scroll.length).toEqual(23)
    expect(@scroll[22]).toEqual("260 X=160")
