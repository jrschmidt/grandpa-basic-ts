...
describe("Test string expression parser", function() {
  ...
  return it("should parse any properly formed string expression", function() {
    var po;
    po = this.parser.string_value_parse('$R');
    expect(po).toEqual(jasmine.any(Array));
    expect(po[0]).toEqual("<string_variable>");
    expect(po[1]).toEqual("R");
    po = this.parser.string_value_parse('$T8');
    expect(po).toEqual(jasmine.any(Array));
    expect(po[0]).toEqual("<string_variable>");
    expect(po[1]).toEqual("T8");
    po = this.parser.string_value_parse('$X0');
    expect(po).toEqual(jasmine.any(Array));
    expect(po[0]).toEqual("<string_variable>");
    expect(po[1]).toEqual("X0");
    po = this.parser.string_value_parse('"HUMMINGBIRD"');
    expect(po).toEqual(jasmine.any(Array));
    expect(po[0]).toEqual("<string_literal>");
    expect(po[1]).toEqual("HUMMINGBIRD");
    po = this.parser.string_value_parse('"ABCDEFGHIJKLMNOPQRSTUVWXYZ"');
    expect(po).toEqual(jasmine.any(Array));
    expect(po[0]).toEqual("<string_literal>");
    expect(po[1]).toEqual("ABCDEFGHIJKLMNOPQRSTUVWXYZ");
    po = this.parser.string_value_parse('"LAST NAME: "+$N2');
    expect(po).toEqual(jasmine.any(Array));
    expect(po[0]).toEqual("<string_literal>");
    ...
    po = this.parser.string_value_parse('"FIRST NAME: "+$N0+";  LAST NAME: "+$N2');
    expect(po).toEqual(jasmine.any(Array));
    expect(po[0]).toEqual("<string_literal>");
    ...
    po = this.parser.string_value_parse('$C2+" IS IN "+$D2');
    expect(po).toEqual(jasmine.any(Array));
    expect(po[0]).toEqual("<string_variable>");
